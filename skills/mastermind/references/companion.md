...
Browser-based visual companion for showing mockups, diagrams, and clickable options (from obra/superpowers, bundled into mastermind at `scripts/companion/`).

## Mastermind usage

- **Phase 1 (discovery):** visual questions only — "which of these 3 layouts feels right?" as clickable cards. Conceptual/scope questions stay in the terminal (see the per-question test below).
- **Phase 6 (design):** show 2–3 candidate design directions (palette + type + hero composition side-by-side); the user's click picks the direction that DESIGN.md commits to.
- **Phase 7 (wireframes):** serve the wireframe pages through the companion instead of bare file-open; the click events give structured feedback per page.
- Requires Node.js and a bash shell (Git Bash on Windows). **Ask the user once before first start** ("want a browser companion for the visual choices?"); if declined or Node missing, fall back to AskUserQuestion in the terminal / plain file-open for wireframes. Never block a phase on the companion.
- **Windows:** the start script runs foreground — launch it with the Bash tool and `run_in_background: true`, then read `$STATE_DIR/server-info` for the URL.
...
**Content fragments vs full documents:** If your HTML file starts with `<!DOCTYPE` or `<html`, the server serves it as-is (just injects the helper script). Otherwise, the server automatically wraps your content in the frame template â€” adding the header, CSS theme, connection status, and all interactive infrastructure. **Write content fragments by default.** Only write full documents when you need complete control over the page.
...
"$HOME/.claude/skills/mastermind/scripts/companion/start-server.sh" --project-dir /path/to/project --open
...
Save `screen_dir` and `state_dir` from the response. With `--open`, the browser opens itself when you push the first screen â€” you don't need to ask the user to open it, but still share the URL as a fallback (headless/remote setups won't auto-open).

**The URL contains a session key (`?key=â€¦`).** The server rejects any request
without it, so always give the user the **complete** URL from the `url` field â€”
...
"$HOME/.claude/skills/mastermind/scripts/companion/start-server.sh" --project-dir /path/to/project --open --foreground
```
...
```bash
"$HOME/.claude/skills/mastermind/scripts/companion/start-server.sh" \
...
1. **Check server is alive**, then **write HTML** to a new file in `screen_dir`:
   - **Required: confirm the server is alive before referring to the URL or pushing a screen.** Check that `$STATE_DIR/server-info` exists and `$STATE_DIR/server-stopped` does not. If it has shut down, restart it with `start-server.sh` using the **same `--project-dir`** â€” it reuses the same port, so the user's open tab reconnects on its own (it shows a "paused" overlay while the server is down) and you don't need to send a new URL. The server auto-exits after 4 hours idle (configurable with `--idle-timeout-minutes`).
   - Use semantic filenames: `platform.html`, `visual-style.html`, `layout.html`
   - **Never reuse filenames** â€” each screen gets a fresh file
   - Use your file-creation tool â€” **never use cat/heredoc** (dumps noise into terminal)
...
3. **On your next turn** â€” after the user responds in the terminal:
   - Read `$STATE_DIR/events` if it exists â€” this contains the user's browser interactions (clicks, selections) as JSON lines
...
If `$STATE_DIR/events` doesn't exist, the user didn't interact with the browser â€” use only their terminal text.
...
```bash
"$HOME/.claude/skills/mastermind/scripts/companion/stop-server.sh" $SESSION_DIR
```
...
- Frame template (CSS reference): `scripts/companion/frame-template.html`
- Helper script (client-side): `scripts/companion/helper.js`
