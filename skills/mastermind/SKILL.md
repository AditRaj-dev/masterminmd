...
2b. **One phase per session.** An approved gate ends the *session*, not just the phase: update memory docs, write `docs/memory/RESUME.md`, mirror to memex, put the resume prompt on the clipboard, tell the user to paste it into a new session, stop. Phase 8 breaks the same way at every parallel group or ~5 accepted tasks. Procedure + the per-phase read-set table: `references/session-handoff.md` — read it at the first gate.
...
- `continue` (or invoked in a project with existing `docs/`) — read `docs/memory/HANDOFF.md` + `MEMORY.md`, then **only** the docs the read-set table in `references/session-handoff.md` lists for the phase you are starting. Report where things stand, resume at the first incomplete phase/task. Do not replay earlier phases' documents.
